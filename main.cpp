...
{
	std::cout << "is_power_of_two(" << value << ") : where the value is of type " << typeid( value ).name() << " = "
						<< bool_to_string( is_power_of_two( value ) ) << std::endl;
}

template < typename... Ts > std::string stringformat( const std::string &fmt, Ts... args )
{
...
int main( int, char ** )
{
	...
	using var_numbers =
			std::variant< unsigned char, char, int, unsigned int, float, double, long double, long long, unsigned long long >;

	std::vector< var_numbers > nums{ -16,   -16.0, 0,    1,  1.0,  8,   8.0,   39,   39.0,   128,   128.5,
	                                 128.0, 1u,    1.0f, 8u, 8.0f, 39u, 39.0f, 128u, 128.5f, 128.0f };

	auto ints{ nums | views::filter( []( const auto &n ) { return std::holds_alternative< int >( n ); } ) |
	           views::transform( []( const auto &n ) { return std::get< int >( n ); } ) };

	auto dbls{ nums | views::filter( []( const auto &n ) { return std::holds_alternative< double >( n ); } ) |
	...
	auto result{ nums | views::filter( []( const auto &value ) {
								 return std::visit( []( const auto &x ) -> bool { return is_power_of_two( x ); }, value );
							 } ) };
	...
	for( const auto &value: result_vec ) {
		std::visit( []( const auto &x ) { std::cout << "Power of two: " << x << std::endl; }, value );
	}
	...
	std::cout << "🌄String format: " << stringformat( "%s é de valor %f", "PI", 3.14 ) << std::endl;
	...
	auto vet = create_container< std::pmr::vector< std::pmr::string > >( &mem_resource, "Hello", "World",
	                                                                     "Hello Long String", "Another Long String" );
